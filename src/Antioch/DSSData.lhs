...
>     where
>       query = "SELECT sessions.id, sessions.name, sessions.min_duration, sessions.max_duration, sessions.time_between, sessions.frequency, allotment.total_time, allotment.grade, targets.horizontal, targets.vertical, status.enabled, status.authorized, status.backup, sessions.session_type_id FROM sessions, allotment, targets, status WHERE allotment.id = sessions.allotment_id AND targets.session_id = sessions.id AND sessions.status_id = status.id AND sessions.project_id = ?"
>       xs = [toSql projId]
>       toSessionDataList = map toSessionData
>       toSessionData (id:name:mind:maxd:between:freq:time:fltGrade:h:v:e:a:b:sType:[]) = 
>         defaultSession {
>             sId = fromSql id 
>           , sName = fromSql name
...
>           , grade = toGradeType fltGrade --TBF: WTF!
...
> toGradeType sqlGrade = GradeA --TBF: if (fromSql sqlGrade) == 3 then GradeA else GradeB 
>       
